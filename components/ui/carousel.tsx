...
const CarouselDots = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
	...
	return (
		<div ref={ref} className={cn("flex items-center justify-center gap-2 py-4", className)} {...props}>
			{scrollSnaps.map((_, index) => (
				<button
					key={index}
					type="button"
					className={cn("h-2 w-2 rounded-full transition-colors", index === selectedIndex ? "bg-orange-600" : "bg-orange-600/50 hover:bg-orange-600")}
